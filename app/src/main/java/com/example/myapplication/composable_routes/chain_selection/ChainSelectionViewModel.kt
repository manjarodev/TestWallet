...
class ChainSelectionViewModel : ViewModel() {
    ...
    val walletEvents = DappDelegate.wcEventModels.map { walletEvent: Modal.Model? ->
        when (walletEvent) {
            is Modal.Model.ApprovedSession -> Events.SessionApproved
            is Modal.Model.RejectedSession -> Events.SessionRejected
            is Modal.Model.SessionAuthenticateResponse -> {
                if (walletEvent is Modal.Model.SessionAuthenticateResponse.Result) {
                    Events.SessionAuthenticateApproved(if (walletEvent.session == null) "Authenticated successfully!" else null)
                } else {
                    Events.SessionAuthenticateRejected
                }
            }

            is Modal.Model.ExpiredProposal -> Events.ProposalExpired
